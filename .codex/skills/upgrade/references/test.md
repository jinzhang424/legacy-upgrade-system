...
- **No boot/smoke case** — the application is never run per batch: mid-upgrade boot failures are expected with partially-migrated dependencies. The orchestrator runs the shared boot-smoke script (`scripts/boot-smoke.js`, config-driven: `start_cmd`, `health_url`, `smoke_routes`) exactly once as a final application gate after all batches complete (see orchestrator.md Stage 3).
- **`harness/baseline.json`** — produced by running `node harness/run.js --baseline` against the **unchanged** repo. Anything already failing pre-change is marked `known-failing`, so executors can distinguish "I broke it" from "it was always broken." Additionally, run the shared boot-smoke script once against the unchanged repo (skip if the config lacks `start_cmd`/`health_url`) and record its result as a `baseline_boot` data field in `baseline.json` — not a harness case — so the final gate can treat a baseline-known-failing boot as informational rather than blaming the upgrade.

...
9. Check whether `<repo_path>/upgrade.config.json` exists (Glob, do not read it yet) and record `upgrade_config_present`. This file is the repo's validation contract (see `schemas/upgrade-config.schema.json`): build/test/start commands, health URL, smoke routes, services, env. If present, read it once and keep its parsed content for stage handoffs. If absent, Stage 1 will propose one and you will confirm it with the user after the analysis gate.
...
**No polling while waiting.** Every sub-agent invocation is synchronous: the Agent tool call blocks until the sub-agent finishes, and its return is the only completion signal. While a sub-agent is running, issue **zero** tool calls — no `repo-status.js`, no reads of `*.draft.json`, `checkpoints/*`, or `manifest.json`, no "is it done yet" probes of any kind. Each probe adds a full conversation turn and wastes tokens without advancing the stage. Draft and progress files are consulted only **after** the Agent call has returned abnormally (disconnect, timeout, or missing artifact — see Checkpoint & Resume), never while the sub-agent is live. The same discipline applies to long-running shell work anywhere in the pipeline: run it as one synchronous call (`run-gate.js`, `boot-smoke.js`, test commands) and wait for it to return — never start it in the background and poll its status or output.
...
1. Spawn the analysis sub-agent with `briefs/analyze-brief.md` inlined verbatim (see Spawn Protocol), followed by only runtime facts: `upgrade_description`, `excluded_paths`, `repo_path`, `run_id`, `run_artifact_dir`, `gitnexus_enabled`, `upgrade_config_present`, and the 2-3 sentence prior-lessons summary from Startup step 7. Do not pass Mem0 identifiers and do not reference `analyze.md`.
   > **Synchronous invocation required:** do NOT set `run_in_background` on this Agent call. Wait for the Agent tool to return before proceeding to the next step, and issue no tool calls while it runs (see No polling while waiting).
...
4. Gate check rejects if any critical deterministic criterion fails, if `artifact_coverage.confidence` is not `"sufficient"`, or if `affected_files`, `dependency_graph`, `risk_summary`, `coverage_notes`, or `dependency_migration_matrix` is missing.
...
6. **Validation contract gate (once per repo):** if `upgrade_config_present` was false, read `<run_artifact_dir>/upgrade.config.proposed.json` and present it to the user for confirmation or edits. On approval, write the confirmed content to `<repo_path>/upgrade.config.json` (this write is explicitly permitted despite the no-repo-modification rule) and keep the parsed content for stage handoffs. If the analyzer could not propose one and the user cannot supply the fields, warn that the harness gate and the final boot/smoke gate will be skipped this run and record that in the manifest.
...
2. Spawn the planner with `briefs/plan-brief.md` inlined verbatim (see Spawn Protocol), followed by the ImpactReport manifest entry, summary, mandatory slices (or condensed handoff path), relevant file-context digest paths, the prior-lessons summary, `upgrade_description`, `user_constraints`, the confirmed upgrade.config.json content, `repo_path`, `run_id`, and `run_artifact_dir`. Do not pass Mem0 identifiers and do not reference `plan.md`.
   > **Synchronous invocation required:** do NOT set `run_in_background` on this Agent call. Wait for the Agent tool to return before proceeding to the next step, and issue no tool calls while it runs (see No polling while waiting).
3. Mandatory ImpactReport slices: `high_risk`, `direct_usage`, `configuration`, `breaking_changes`, `coverage_notes`, `dependency_summary`, and `migration_matrix`.
...
7. If the gate exits 1 (non-MISSING), do not spawn a fresh planner. The planner session stays available after its final message: send that **same session** one follow-up containing only the `still_failing` list and — for coverage failures (P10/P11) — the uncovered `(dependency, file_path)` pairs, instructing it to update `change-plan.json`, the affected slices, and the manifest entry in place. Fall back to a fresh spawn with a `resume_from_checkpoint` block only if the session is no longer available. If the gate fails a second time, halt and report.
...
4. Require the test planner to write `test-plan.json`, its summary, high-priority test, regression test, framework recommendation slices, the manifest entry, **and the slim, change-agnostic harness**: `<run_artifact_dir>/harness/run.js` — module-load checks for touched modules and one case running the repo's own test suite when a test command exists — plus `<run_artifact_dir>/harness/baseline.json` produced by running the harness once against the unchanged repo (already-failing entries marked known-failing, plus a one-off baseline boot-smoke result recorded as a `baseline_boot` data field for the Stage 3 final gate). The harness contains no characterization cases, no module-loader interception, and no fakes of upgraded dependencies; behavioral coverage comes from the TestPlan's I/O-spec cases, implemented by Stage 4-B. The harness — not executor judgment — is what Stage 3 gates run against.
...
6. If a passed result validates, continue. A `failed` result only arrives after the executor's own in-run patch attempts (recorded in its `notes`, per its brief — see Failure Recovery) are exhausted or out of scope; when a failed result validates, apply rollback using the ChangePlan rollback summary/full artifact as needed, stage only rolled-back paths with exact pathspecs, verify the cached diff path set, commit rollback, notify the user, and halt. If the gate exits 1 (non-MISSING), the mechanical repair already ran inside the gate call; ask the executor to re-emit a valid result for the `still_failing` criteria without re-running changes.
7. **Final application gate (orchestrator-owned, runs once).** Per-batch validation never boots the application — mid-upgrade boot failures are expected with partially-migrated dependencies. After the final batch's ValidationResult passes validation, run the boot+smoke gate exactly once:

   ```text
   node .codex/skills/upgrade/scripts/run-gate.js --run-dir <run_artifact_dir> --stage execute-final --gate boot-smoke -- node .codex/skills/upgrade/scripts/boot-smoke.js --config <upgrade.config.json> --repo <repo_path>
   ```

   Skip the gate — and record the skip in the manifest — when `upgrade.config.json` is absent or lacks `start_cmd`/`health_url` (same degradation as Stage 1 step 6). Exit 0 → proceed to step 8. Non-zero → first check `harness/baseline.json`: if its `baseline_boot` field records that the unchanged repo already failed to boot, the failure is informational — record it in the manifest and the final summary and proceed to step 8 rather than blaming the upgrade. Otherwise route through Failure Recovery's second line: spawn an executor patch invocation with the gate's evidence path, the failing gate, and scope = any executor-owned file across all batches; after 2 failed patch attempts, apply rollback per step 6, notify the user, and halt.
8. On final `status: "passed"` with deterministic approval and the final application gate passed (or skipped/informational), proceed to Stage 4-B before the final user summary.
...
2. Spawn the test implementation sub-agent with `briefs/test-implement-brief.md` inlined verbatim (see Spawn Protocol), followed by the TestPlan manifest entry, summary, high-priority/regression/framework slices (or condensed handoff path), the `slices/execute-*-signature-changes.json` paths collected across all execution batches, `upgrade_description`, `repo_path`, `branch_name`, `run_id`, and `run_artifact_dir`. Do not pass Mem0 identifiers and do not reference `test.md`.
   > **Synchronous invocation required:** do NOT set `run_in_background` on this Agent call. Wait for the Agent tool to return before proceeding to the next step, and issue no tool calls while it runs (see No polling while waiting).
...
6. If the validated result reports `failure_origin: "source_change"` (visible in `summaries/test-result-summary.json` — never read the full artifact), the generated tests are correct and the source change is at fault: spawn an executor patch invocation per Failure Recovery before proceeding.
7. Proceed to Stage 5 to generate the run guide. If test planning failed (`test_plan = null`), still proceed to Stage 5 — the run guide will note that test generation was skipped and will omit test result counts.
...
A failure is delegated to the agent whose output caused it: a source-change error → the executor, a generated-test error → the test implementer, a harness error → the test planner. Patching never restarts a stage or regenerates its artifacts — the responsible agent amends them in place (artifact, summary, slices, manifest entry), staying strictly inside its own write scope.

- **First line — in-run.** Each brief instructs its agent to diagnose and patch a failure inside its own run (at most 2 patch attempts, in scope) before reporting `status: "failed"`: the executor patches in-batch-scope gate failures and re-runs only the failed gate; the test implementer fixes wrong test code (never source) and re-runs only the failing test; the test planner fixes mechanical harness/baseline errors and re-runs the baseline.
- **Second line — orchestrator patch invocation.** When a failure surfaces only after the responsible stage returned (e.g. Stage 4-B reports `failure_origin: "source_change"`, which implicates the executor), spawn a focused patch invocation of the responsible agent: its same brief plus a `patch_mode` block containing the failure evidence paths, the failing gate/test, the affected files, the 2-attempt cap, and the scope restriction (only files that agent owns).
- **Exhaustion.** After 2 failed orchestrator-level patch attempts, fall back to the pre-recovery behavior: rollback per Stage 3 rules / warn per Stage 4-B rules, halt, and notify the user.

## Escalation Rules

- If a sub-agent fails to produce valid output after 2 attempts, halt and report the failed criteria and artifact paths. Runtime failures (gates, tests, harness) route through Failure Recovery first; only validation-output failures use this rule directly.
- Wherever a stage's retry rule says "re-invoke", continue the producing sub-agent's live session with a follow-up message when that session is still available; spawn a fresh agent (with a `resume_from_checkpoint` block) only when it is not.
...
- Polling a running sub-agent — any status probe, checkpoint/draft read, or `repo-status.js` call issued while an Agent invocation is still in flight — is a protocol violation (see No polling while waiting).
...
- The auto-approved tools in `.codex/rules/upgrade-pipeline.rules` (`rg`, `git`, `npx gitnexus`, `npm test`/`npm run`, `npx jest`, `python -m pytest`, `node --check`, the pipeline scripts: validator, fixer, repo-status, run-gate, boot-smoke, and — for Stage 4-B only — dev-dependency install commands such as `npm install --save-dev` and `pip install` for well-known test frameworks and standard test-support libraries, in-memory service packages included) and the auto-approved MCP tools (`gitnexus_query`/`cypher`/`impact`, `mem0_add_memory`/`search_memories`) are approved only for use inside `repo_path` or the run artifact dir. Before invoking any of them with a path, working directory, or target outside `repo_path`, halt and ask the user for explicit approval — do not treat the pre-approval as covering out-of-repo use.

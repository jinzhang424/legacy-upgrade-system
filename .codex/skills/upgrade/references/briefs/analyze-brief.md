...
4. If `<repo_path>/upgrade.config.json` is missing, write a proposal to `<run_artifact_dir>/upgrade.config.proposed.json` (start_cmd, start_cwd, health_url, smoke_routes, test_cmd, services, env) from the entry points, ports, and routes you discover. Include `test_cmd` ONLY if the command is verified to exist (e.g. a `scripts.test` entry in the relevant package manifest, a pytest/tox config, a Makefile test target); never propose a guessed test command — omit the field instead.
